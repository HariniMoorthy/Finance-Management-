    ...
    public class BlazorCookieLoginMiddleware
    {
        ...
        public async Task Invoke(HttpContext context, SignInManager<ApplicationUser> signInMgr, UserManager<ApplicationUser> UserManager)
        {
            if (context.Request.Path == "/login" && context.Request.Query.ContainsKey("key"))
            {
                ...
                var info = Logins[key];
                var usr = await UserManager.FindByEmailAsync(info.Email);
                var result = await signInMgr.PasswordSignInAsync(user: usr, info.Password, false, lockoutOnFailure: true);
                info.Password = null;
                if (result.Succeeded)
                {
                    Logins.Remove(key);
